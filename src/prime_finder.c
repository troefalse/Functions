...
void old_append_if_prime(int primecandidate, int * primelist, int *length){
	int i;
	for (i=0; i<(*length); i++){
...
int main(){
	...
	for(int candidate=3; candidate<MAXIMUM_INPUT; candidate+=2){
		...
		// old_append_if_prime(candidate, primenumbers, &current_length);
	}
